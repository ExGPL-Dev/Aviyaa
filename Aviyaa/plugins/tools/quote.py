...
async def get_message_sender_name(ctx: Message):
    def clean_name(name):
        if not name:
            return ""
        # Remove special characters that might break the quote API
        ...
        if ctx.forward_date:
            if ctx.forward_sender_name:
                ...
                if ctx.forward_from.last_name:
                    return clean_name(f"{ctx.forward_from.first_name} {ctx.forward_from.last_name}")
                else:
                    return clean_name(ctx.forward_from.first_name or "")
            elif ctx.forward_from_chat:
                return clean_name(ctx.forward_from_chat.title or "")
        elif ctx.from_user:
            if ctx.from_user.last_name:
                return clean_name(f"{ctx.from_user.first_name} {ctx.from_user.last_name}")
            else:
                return clean_name(ctx.from_user.first_name or "")
        elif ctx.sender_chat:
...
async def pyrogram_to_quotly(messages, is_reply):
    ...
    # Solution for pixelation - increase quality parameters
    payload = {
        "type": "quote",
        "format": "png",
    ...
    for message in messages:
        ...
        try:
            ...
            the_message_dict_to_append["from"]["type"] = message.chat.type.name.lower() if message.chat else "private"
            the_message_dict_to_append["from"]["photo"] = await get_message_sender_photo(message)
            
            # Solution for reply messages not working - better error handling
