...
# Entity mapping from the other plugin
_entities = {
...
async def get_message_sender_name(ctx: Message):
    def clean_name(name):
        if not name:
            return "Deleted Account"  # Default name like the other plugin
        # Properly handle HTML special characters and emojis
    ...
    try:
        if ctx.forward_date:
            if (
                ...
                return None  # Changed to None to match other plugin
            else:
...
async def get_text_or_caption(ctx: Message):
    try:
        ...
        # Clean the text to prevent rendering issues
...
async def pyrogram_to_quotly(messages, is_reply):
    ...
    # Using parameters from the other plugin for better quality
    payload = {
        "type": "quote",
        "format": "webp",  # Changed to webp for better quality
        "backgroundColor": "#1b1429",
        "width": 512,      # Added from other plugin
        "height": 768,     # Added from other plugin
        "scale": 2,        # Added from other plugin
        "messages": [],
    }
    ...
    for message in messages:
        ...
        try:
            # Using the entity mapping from the other plugin
            if message.entities:
                the_message_dict_to_append["entities"] = [
                    {
                        "type": _entities.get(type(entity), entity.type.name.lower()),
            ...
            if message.reply_to_message and is_reply:
                try:
                    ...
                    # If reply message processing fails, skip it rather than failing entirely
                    the_message_dict_to_append["replyMessage"] = {}
            else:
                the_message_dict_to_append["replyMessage"] = {}
    ...
    try:
        # Try both APIs like the other plugin does
        urls_to_try = [
    ...
    if len(ctx.text.split()) > 1:
        ...
        if check_arg[0]:
            ...
            try:
                ...
                bio_sticker.name = "quote.webp"  # Changed to webp
